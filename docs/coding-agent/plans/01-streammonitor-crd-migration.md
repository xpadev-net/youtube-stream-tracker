...
- [x] (2026-08-21) Step 1: `helm/stream-monitor/crds/streammonitor-crd.yaml` created exactly as specified; validated with `kubectl apply --dry-run=client` and `helm lint`/`helm template`.
- [x] (2026-08-21) Step 2: `internal/model/model.go` created (package `model`), moved from `internal/db/models.go` minus `MonitorEvent`/`WebhookStatus`; added `Monitor.UID types.UID`.
- [x] (2026-08-21) Step 3: `internal/k8s/apis/streamtracker/v1alpha1/types.go` created with hand-written `DeepCopyObject()` via JSON round-trip.
- [x] (2026-08-21) Step 4: `internal/k8s/store` package created (`store.go`, `convert.go`, `store_test.go`) with the full method set, informer-backed reads, live writes, and the two-step Create (status subresource requires a separate `UpdateStatus` call after `Create`). Exercised against `k8s.io/client-go/dynamic/fake` — no real cluster.
- [x] (2026-08-21) Step 5: `model.StatusScheduled` added to the enum without touching `IsActive()`.
- [x] (2026-08-21) Step 6: `internal/k8s/k8s.go` updated — `CreatePodParams.Config` is now `*model.MonitorConfig`, added `OwnerUID`/`OwnerName`, worker Pod `ownerReferences` now point at the `StreamMonitor`. Deleted `Client.ownerRef`, `SetOwnerReference`, `ResolveOwnerDeployment`, `buildOwnerReferences`, `findOwnerReference`, `buildDeploymentOwnerReference`, `BuildOwnerReference`. Added `k8s.BuildRESTConfig` shared by the typed clientset and the new dynamic client.
- [x] (2026-08-21) Step 7: `internal/k8s/reconcile.go` and `internal/k8s/watcher.go` updated to use `*store.Store`/`model.*`; audit-trail (`MonitorEvent`) blocks replaced with local `zap` log lines; `CreateMonitorPod` now passes `OwnerUID`/`OwnerName`.
- [x] (2026-08-21) Step 8: `internal/api/handlers.go` updated to `*store.Store`/`model.*`; `RecordWebhookEvent`/`ListEvents` handlers and their request/response types deleted.
- [x] (2026-08-21) Step 9: `cmd/gateway/main.go` rewritten — builds a `*rest.Config` via `k8s.BuildRESTConfig`, constructs `dynamic.Interface`, starts `store.Store.Run`/`WaitForCacheSync`; removed the `ResolveOwnerDeployment`/`SetOwnerReference` block, `runCleanupLoop` and its goroutine/config, and the two deleted audit-log routes; `readyzHandler` now checks `store.WaitForCacheSync` with a short timeout instead of `database.Health`.
- [x] (2026-08-21) Step 10: `internal/worker/worker.go` and `internal/worker/callback.go` now import `internal/model` instead of `internal/db`; deleted `WebhookEventReport`, `CallbackReporter.ReportWebhookEvent` and its implementation, the post-webhook audit POST call site (kept the webhook-send call itself), and the now-dead `auditWg`/`deepCopyMap`/`shallowCopyMap` helpers that existed solely to support it. `internal/worker/worker_test.go` updated to match (removed `spyCallbackClient.ReportWebhookEvent`). `internal/config/config.go`'s `LoadWorkerConfig` now imports `internal/model`.
- [x] (2026-08-21) Step 11: `internal/db/` deleted in full (`git rm -r`); `GatewayConfig.DatabaseURL`/`MonitorRetentionPeriod`/`CleanupInterval` and the `DB_DSN`/`DATABASE_URL` required-field check removed from `internal/config/config.go`; `docker-compose.yaml`'s `postgres` service and the gateway's `DATABASE_URL`/`depends_on` removed; `helm/stream-monitor/templates/deployment.yaml`'s `DATABASE_PASSWORD`/`DB_DSN`/`DATABASE_URL` env blocks removed; `helm/stream-monitor/values.yaml`'s `postgresql:` section removed; `helm/stream-monitor/templates/_helpers.tpl`'s `stream-monitor.databaseURL` define removed; `helm/stream-monitor/templates/rbac.yaml`'s Deployment/ReplicaSet rules replaced with the `streammonitors`/`streammonitors/status` rule. `go mod tidy` run — `github.com/jackc/pgx/v5` and its transitive deps (`pgpassfile`, `pgservicefile`, `puddle/v2`) confirmed absent from `go.mod`/`go.sum`. `grep -rl '"github.com/xpadev-net/youtube-stream-tracker/internal/db"' --include='*.go' .` returns no results. `README.md` updated to remove `DB_DSN`/`DATABASE_URL`/Postgres mentions.
- [x] (2026-08-21) `go build ./...`, `go vet ./...`, and `go test ./...` all pass (95 tests across 18 packages, including the new `internal/k8s/store` tests and the pre-existing `internal/worker` tests). `helm lint`/`helm template`/`kubectl apply --dry-run=client` all pass against the new chart and CRD.
- [ ] Concrete Steps 3–15 (real `kind` cluster: `kind create cluster`, `helm install`, `curl` against a live Gateway Pod, `kubectl get streammonitors`, duplicate-409 check, delete-cascades-to-Pod check) **not yet run** — this sandboxed environment has `kubectl`/`helm` but no working Docker daemon (`docker info` fails) and no `kind` binary installed, so a real cluster cannot be created here. See Surprises & Discoveries below for what was validated instead, and Outcomes & Retrospective for exactly what a human/agent with Docker+kind must still run to close out this plan.

## Surprises & Discoveries

- Observation: `helm template` (without `--show-only`) appeared in this sandboxed terminal to only render 3 of the chart's 6 templates (`serviceaccount.yaml`, `secret.yaml`, `configmap.yaml` — `deployment.yaml`, `service.yaml`, `rbac.yaml` seemed to vanish), which looked like a real templating bug at first.
  Evidence: `helm template stream-monitor helm/stream-monitor ... > /tmp/full.yaml; wc -l /tmp/full.yaml` reported only 41 lines / 3 `kind:` lines, with no error from `--debug`. But `helm template ... --show-only templates/deployment.yaml` (and `rbac.yaml`, `service.yaml`) each rendered correctly and completely on their own. The full, un-filtered `helm template` output is large enough that this session's own command-output summarization (unrelated to Helm) appears to have trimmed it before it reached this transcript — the file on disk was checked via `grep -c`/`wc -l` after the fact rather than trusting the transcript's inline echo. Conclusion: not a real chart bug; `--show-only` per-template checks plus `helm lint` are what actually validate the chart in this environment.
- Observation: the informer-backed `Store` reads are eventually consistent even inside a single test process using the fake dynamic client — a live write via `UpdateStatusWithCondition` is not immediately visible to a subsequent `GetByID` (which reads the informer's local cache), because the fake client's Watch delivers the update asynchronously just like a real cluster would.
  Evidence: `store_test.go`'s `TestUpdateStatusWithCondition` initially failed with `Status = initializing, want waiting` when asserting immediately after the write; polling `GetByID` in a short loop (as the rest of the test suite already did via `waitInCache`) fixed it. This is expected/correct informer behavior, not a bug — documented via the Decision Log's "duplicate-active-stream_url" entry already covers the general trade-off — this is just where a test had to account for it explicitly.
...
- Decision: This ExecPlan's "Concrete Steps" 3–15 (creating a real `kind` cluster and validating end-to-end against it) could not be executed inside this implementation session's sandbox, because it has no working Docker daemon (`docker info` fails) and no `kind` binary. All code-level validation that does not require a live cluster was still performed here: `go build ./...`, `go vet ./...`, `go test ./...` (including new `internal/k8s/store` tests against `k8s.io/client-go/dynamic/fake`), `kubectl apply --dry-run=client` against the CRD manifest, and `helm lint`/`helm template --show-only <template>` against every template in the chart.
  Rationale: this is an environment limitation, not a decision to skip validation — the plan's own Concrete Steps already anticipate a novice needs to install `kind`/`kubectl` first (step 3), which implies these are not assumed to be preinstalled everywhere the plan is followed.
  Date/Author: implementation session, 2026-08-21.
...
**Upgrading an existing (pre-CRD) release.** Concrete Step 5 (`kubectl apply -f helm/stream-monitor/crds/streammonitor-crd.yaml`) matters even more, not less, if you already have a running `stream-monitor` Helm release from before this plan (the old Postgres-backed version): `helm upgrade` never installs or updates anything in a chart's `crds/` directory (see the caveat already called out in "Plan of Work" step 1). If you run `helm upgrade` to move an existing release onto this version of the chart without first running that `kubectl apply` command, the CRD will not exist yet, and the new Gateway image will crash-loop (`kubectl describe pod` will show `readyz`/`healthz` failures) because it lists/watches `StreamMonitor` objects at startup and there is no such resource type registered in the cluster. The fix is exactly Concrete Step 5, run once, before or immediately after the `helm upgrade` — order between the two does not matter, since the Gateway Pod will simply keep restarting until the CRD exists, and once it does, the next restart succeeds. This is safe to re-run and does not touch any other cluster state.

## Outcomes & Retrospective

As of 2026-08-21, every source-code and packaging change described in "Plan of Work" (steps 1 through 11) is complete and merged into the working tree: there is no `internal/db` package, no `pgx` dependency, no Postgres in `docker-compose.yaml` or the Helm chart, and every monitor is represented purely as a `StreamMonitor` custom resource read/written through `internal/k8s/store.Store`. `go build ./...`, `go vet ./...`, and `go test ./...` all pass (95 tests, 18 packages), and `helm lint` plus per-template `helm template --show-only` and `kubectl apply --dry-run=client` against the CRD all pass.

What remains, and is explicitly out of scope for this implementation session to close out itself: Concrete Steps 3 through 15, which require a real local Kubernetes cluster (`kind create cluster`), building and loading the gateway/worker container images, `helm install`, and a live `curl` walkthrough of create/get/duplicate-409/delete against a running Gateway Pod. This session's sandbox has `kubectl` and `helm` but no working Docker daemon and no `kind` binary, so these steps could not be executed here (see the Decision Log entry above). Before this plan can be marked fully accepted per "Validation and Acceptance," a human or agent with Docker + `kind` available must run Concrete Steps 3–15 verbatim and confirm each stated observation, in particular: step 11 (`kubectl get streammonitors` shows the object with a populated `Phase` column — this is the two-step Create-then-UpdateStatus behavior in `store.Store.Create` working against a real API server, not just the fake client used in `store_test.go`), step 12 (the worker Pod's `ownerReferences` show `"kind":"StreamMonitor"`), step 13 (a duplicate `POST` for the same `stream_url` returns `409`), and step 14 (deleting the monitor cascades to deleting the Pod via Kubernetes' own garbage collector, not just the explicit `DeleteWorkerPod` call). Only after that live-cluster pass should this plan be considered fully accepted and plan 2 started.

Comparing the result against the original Purpose: the code-level goal — "there is no database anywhere in the system... you will be able to run `kubectl get streammonitors`" — is achieved in the source tree and validated as far as static tooling (build/vet/test/lint/dry-run) can confirm; the remaining gap is purely the live-cluster demonstration, not any known code defect.
...
Implemented steps 1–11 of "Plan of Work" in full (all Postgres/`internal/db` code, config, Docker Compose, and Helm chart references removed; the new `internal/model`, `internal/k8s/apis/streamtracker/v1alpha1`, and `internal/k8s/store` packages added; `internal/api`, `internal/k8s`, `internal/worker`, `internal/config`, and `cmd/gateway/main.go` all migrated off `internal/db` onto the new `model`/`store` types). `go build`/`go vet`/`go test` all pass, and the Helm chart plus CRD were validated with `helm lint`, `helm template --show-only <template>` per template, and `kubectl apply --dry-run=client`. Concrete Steps 3–15 (the real `kind`-cluster walkthrough) were not run in this session because the sandbox has no working Docker daemon and no `kind` binary — see the Decision Log and Outcomes & Retrospective sections above for what was validated instead and what a follow-up session with Docker/`kind` available must still do to fully close out this plan's acceptance criteria.

## Revision Note (2026-08-21, second pass — code-review hardening)

Automated code review (CodeRabbit, Greptile) on the pull request surfaced several genuine correctness issues in `internal/k8s/store/store.go` and related files, all fixed in this pass: `Create` now deletes the just-created `StreamMonitor` object if the follow-up `UpdateStatus` call fails, instead of leaving a `status.phase`-empty orphan that was invisible to the phase index and to the duplicate-stream-URL check; `List`'s pagination clamps a negative `Offset` to zero instead of panicking on `monitors[start:end]` (`internal/api/handlers.go`'s `ListMonitors` also now rejects a negative `?offset=` query parameter outright, at the API boundary); `UpdateStatus`/`UpdatePodName`/`UpdateStats`/`UpdateMonitor` now wrap their read-modify-write cycle in `k8s.io/client-go/util/retry.RetryOnConflict`, re-reading the live object on every attempt, so a concurrent writer (the worker's status callback, the reconciler, and the pod watcher can all touch the same object) causes a transparent retry instead of an HTTP 409 Conflict surfacing to the caller — this is a behavior `UpdateStatusWithCondition` deliberately does not get, since a 409 there correctly means "someone else already handled this transition," per its existing doc comment; `UpdateStats` no longer writes an empty string for `videoHealth`/`audioHealth`/`streamStatus` when a partial status-update request omits them (the CRD schema's enum constraint would otherwise reject the whole write); `internal/k8s.Client.CreateWorkerPod` now fails fast with a clear error when `OwnerUID` is empty (Kubernetes itself rejects an `ownerReferences` entry with an empty `uid`) and builds the ownerReference's `apiVersion`/`kind` from the `v1alpha1` package's own constants instead of duplicating those strings; `v1alpha1.StreamMonitor{,List}.DeepCopyObject` no longer silently discards a `json.Marshal`/`Unmarshal` error (it panics instead, since such an error is only reachable via a bug in this package's own types, never bad cluster/user input — the JSON-round-trip design itself, recorded in the Decision Log, is unchanged); and this plan's own "Idempotence and Recovery" section plus the CRD manifest itself now call out that upgrading an existing (pre-CRD) Helm release requires running `kubectl apply -f helm/stream-monitor/crds/streammonitor-crd.yaml` once, since `helm upgrade` never touches a chart's `crds/` directory. Also fixed: two flaky `internal/k8s/store` tests that didn't account for the informer cache's asynchronous catch-up after a live write (replaced with a shared `waitForStatus` polling helper), and stale README wording left over from the Postgres-era gateway description. `go build`/`go vet`/`go test ./...` all still pass (97 tests, 18 packages), plus `go test ./internal/k8s/... -count=15 -race`.

Two review findings were deliberately left as-is, with a reply on the PR thread explaining why rather than a code change: (1) the informer-cache duplicate-stream-URL check has an inherent race window under near-simultaneous concurrent creates for the same stream URL — this is the exact, explicitly-accepted trade-off already recorded in this Decision Log's "Duplicate-active-stream_url prevention..." entry, which says not to fix it with a live read without discussing the performance trade-off first; and (2) `internal/api/handlers_test.go`'s use of a zero-value `*store.Store`/`*kubernetes.Clientset` (rather than injectable fake interfaces) limits those tests to structural/validation-path coverage — a real fix requires extracting narrow interfaces for `Handler`'s and `Client`'s dependencies, which is a genuine but separate refactor (a "heavy lift" per the review itself) better done deliberately than as a drive-by in a review-response pass.
